...
template <typename T> T clampValue(const T &v, const T &lo, const T &hi) {
  return (v < lo) ? lo : (v > hi) ? hi : v;
}

Car::Car(std::shared_ptr<Model> bodyModel, std::shared_ptr<Model> wheelModel,
         std::shared_ptr<Model> steeringModel) {
  ...
  wheels[0] = std::make_unique<Wheel>(std::make_shared<Model>(*wheelModel),
                                      WheelPos::RearRight);
  wheels[1] = std::make_unique<Wheel>(std::make_shared<Model>(*wheelModel),
                                      WheelPos::RearLeft);
  wheels[2] = std::make_unique<Wheel>(std::make_shared<Model>(*wheelModel),
                                      WheelPos::FrontRight);
  wheels[3] = std::make_unique<Wheel>(std::make_shared<Model>(*wheelModel),
                                      WheelPos::FrontLeft);
  ...
  wheelRotationOffsets[0] =
      physx::PxQuat(glm::radians(-90.0f), physx::PxVec3(0, 1, 0)); // RR
  wheelRotationOffsets[1] =
      physx::PxQuat(glm::radians(90.0f), physx::PxVec3(0, 1, 0)); // RL
  wheelRotationOffsets[2] =
      physx::PxQuat(glm::radians(-90.0f), physx::PxVec3(0, 1, 0)); // FR
  wheelRotationOffsets[3] =
      physx::PxQuat(glm::radians(90.0f), physx::PxVec3(0, 1, 0)); // FL
}
...
void Car::Update(float dt, glm::vec3 position, physx::PxQuat rotation,
  ...
  if (steeringWheelVisualMaxAngle > 0.0f) {
    targetSteeringWheelAngle =
        glm::clamp(targetSteeringWheelAngle, -steeringWheelVisualMaxAngle,
                   steeringWheelVisualMaxAngle);
  }

  float alpha = glm::clamp(steeringWheelVisualSmooth * dt, 0.0f, 1.0f);
  steeringWheelVisualAngle =
      glm::mix(steeringWheelVisualAngle, targetSteeringWheelAngle, alpha);

  float steeringWheelAngleDeg = steeringWheelVisualAngle;

  glm::quat steeringTurn =
      glm::angleAxis(glm::radians(steeringWheelAngleDeg), glm::vec3(1, 0, 0));
