...
bool TireSquealSound::load(const char* path) {
    ...
    if (result != MA_SUCCESS) {
        std::cerr << "TireSquealSound: failed to load " << path << ", code = " << result << std::endl;
        return false;
    }
...
void TireSquealSound::update(float tireForwardSlip, float tireSideSlip, float speed) {
    ...
    float volFactor = (dv > 0.0f) ? volumeSmoothFactor * attackMult : volumeSmoothFactor * releaseMult;
    ...
    float pitchFactor = (dp > 0.0f) ? pitchSmoothFactor * pitchAttack : pitchSmoothFactor * pitchRelease;
