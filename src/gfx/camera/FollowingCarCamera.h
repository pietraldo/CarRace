...
public:
    ...
    FollowingCarCamera(glm::vec3 position = glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3 up = glm::vec3(0.0f, 1.0f, 0.0f),
                       float yaw = YAW, float pitch = PITCH)
        : Camera(CameraType::FOLLOWING_CAR_CAMERA, position, up, yaw, pitch) {}

    // constructor with scalar values
    FollowingCarCamera(float posX, float posY, float posZ, float upX, float upY, float upZ, float yaw, float pitch)
    ...
    void Update(const glm::vec3& carPosition, const glm::quat& carRotation) {
        ...
        if (y_car_rotation < 0.05f) y_car_rotation = 0.0f;
        forward.y = 0.0f;
