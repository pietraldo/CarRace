...
protected:
  ...
  Camera(CameraType cameraType, glm::vec3 position, glm::vec3 up, float yaw,
         float pitch)
      : Position(position), Front(glm::vec3(0.0f, 0.0f, -1.0f)), WorldUp(up),
        Yaw(yaw), Pitch(pitch), MovementSpeed(SPEED), Zoom(ZOOM),
        cameraType(cameraType) {
    updateCameraVectors();
  }

  // constructor with scalar values
  Camera(CameraType cameraType, float posX, float posY, float posZ, float upX,
         float upY, float upZ, float yaw, float pitch)
      : Front(glm::vec3(0.0f, 0.0f, -1.0f)), MovementSpeed(SPEED), Zoom(ZOOM),
        cameraType(cameraType), Position(glm::vec3(posX, posY, posZ)),
