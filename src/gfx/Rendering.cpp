...
int Rendering::Initialize() {
  ...
  textureData = stbi_load("../assets/vehicledata/baseColor6.png", &texWidth,
...
void Rendering::RenderImGui() {
    ...
    if (currentMode == ViewMode::SINGLE_SCREEN) {
      ...
      if (activeCamera == CameraType::FIRST_PERSON_CAMERA) {
        FirstPersonCamera *fpCam =
            dynamic_cast<FirstPersonCamera *>(&activeCam);
        if (fpCam) {
  ...
  }
  {
    ...
    ImGui::SliderFloat("Quadratic", &(*gameEngine).flashlight->quadratic, 0,
                       0.1);
    ImGui::SliderFloat("CutOff", &(*gameEngine).flashlight->cutOff, 0.9, 1);
    ImGui::SliderFloat("OuterCutOff", &(*gameEngine).flashlight->outerCutOff,
                       0.9, 1);
    ImGui::End();
  }
  {
    ...
    ImGui::DragFloat("PositionX", &(*gameEngine).cube->positionToDisplay.x,
                     sensitivity);
    ImGui::DragFloat("PositionY", &(*gameEngine).cube->positionToDisplay.y,
                     sensitivity);
    ImGui::DragFloat("PositionZ", &(*gameEngine).cube->positionToDisplay.z,
                     sensitivity);
    ImGui::DragFloat("RotationX", &(*gameEngine).cube->rotationToDisplay.x,
                     sensitivity);
    ImGui::DragFloat("RotationY", &(*gameEngine).cube->rotationToDisplay.y,
                     sensitivity);
    ImGui::DragFloat("RotationZ", &(*gameEngine).cube->rotationToDisplay.z,
                     sensitivity);

    ImGui::Text("Scale: x: %.2f y: %.2f z: %.2f", (*gameEngine).cube->scale.x,
                (*gameEngine).cube->scale.y, (*gameEngine).cube->scale.z);
  ...
  }
  {
    if (!gameEngine->modelsTex.empty()) {
      ...
      ImGui::SliderFloat("Adjust Sensitivity", &modelSensitivity, 0.001f,
                         10.0f);
...

  {
    ...
    if (car) {
      ...
      float sideCoeff = (sideSign < 0.0f) ? Mirrors::mirrorLookSide
                                          : -Mirrors::mirrorLookSide;
      glm::vec3 mirrorDir = glm::normalize(-forward + right * sideCoeff +
                                           up * Mirrors::mirrorLookUp);

      ImGui::Text("Left Mirror Pos: %.2f %.2f %.2f", mirrorPos.x, mirrorPos.y,
                  mirrorPos.z);
      ImGui::Text("Left Mirror Dir: %.2f %.2f %.2f", mirrorDir.x, mirrorDir.y,
                  mirrorDir.z);
    }
...
void Rendering::RenderFrame(std::vector<GameObject *> gameObjects) {
  ...
  if (currentViewMode == ViewMode::EDIT_SCREEN) {
    ...
    if (gameEngine->renderMirrors &&
        activeCam.cameraType == CameraType::FIRST_PERSON_CAMERA) {
